...
const typographyVariants = cva('text-foreground', {
  variants: {
    variant: {
      ...
      h3: 'scroll-m-20 text-zinc-300 text-2xl font-semibold tracking-tight',
      h4: 'scroll-m-20 text-zinc-400 text-xl font-semibold tracking-tight',
...
function Typography<T extends Element>(
  { className, as, variant, ...props }: TypographyProps<T>,
  ref: React.ForwardedRef<T>
) {
  const Component = as ?? (variant ? variantElementMap[variant] : undefined) ?? 'div'
